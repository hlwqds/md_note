...
func dirList(w http.ResponseWriter, r *http.Request, f os.File) {
	...
	for _, d := range dirs {
		name := d.Name()
		if d.IsDir() {
			name = ModuleSkyDrive + "/" + name + "/"
		} else {
			name = ModuleSkyDrive + SubModuleSkyDriveFile + "/" + name
		}
		// name may contain '?' or '#', which must be escaped to remain
		// part of the URL path, and not indicate the start of a query
		// string or fragment.
		url := url.URL{Path: name}
		fmt.Fprintf(w, "<a href=\"%s\">%s</a>", url.String(), html.EscapeString(d.Name()))

		fmt.Fprintf(w, "\t%2v-%3v-%4v\t%20vKB\n", d.ModTime().Day(), d.ModTime().Month(), d.ModTime().Year(), d.Size()/(1<<10))
	}
	fmt.Fprintf(w, "</pre>\n")
}
